...
export default function AccesoriosVariants({
  variants,
  setVariants,
  fieldErrors,
}) {
  const handleAddVariant = useCallback(() => {
    setVariants((prev) => [
      ...prev,
      { nombre: "", valor: "", cantidad: 0, imagenes: [] },
    ]);
  }, [setVariants]);
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Define los atributos, cantidad y SKU para cada variante del
            accesorio.
          </p>
        </div>
        <Button
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div className="grid flex-1 gap-4 sm:grid-cols-2">
                  <div className="space-y-2">
                    <Label htmlFor={`variant-nombre-${index}`}>Atributo</Label>
                    <Input
                      id={`variant-nombre-${index}`}
                      value={variant.nombre}
                      onChange={(event) =>
                        updateVariantField(index, "nombre", event.target.value)
                      }
                      placeholder="Ej. Material"
                      aria-invalid={Boolean(variantError.nombre)}
                    />
                    {variantError.nombre ? (
                  ...
                  <div className="space-y-2">
                    <Label htmlFor={`variant-valor-${index}`}>Valor</Label>
                    <Input
                      id={`variant-valor-${index}`}
                      value={variant.valor}
                      onChange={(event) =>
                        updateVariantField(index, "valor", event.target.value)
                      }
                      placeholder="Ej. Acero inoxidable"
                      aria-invalid={Boolean(variantError.valor)}
                    />
                    {variantError.valor ? (
                      <p className="text-xs text-red-500">
                        {variantError.valor}
